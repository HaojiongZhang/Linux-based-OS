...
void entry(unsigned long magic, unsigned long addr) {
    ...
    if (CHECK_FLAG(mbi->flags, 3)) {
        ...
        fs_init((boot_block_t*)mod->mod_start);             //initialize file system
    ...
    init_terminal();
    paging_init();
    pit_init(); //after initializing pit, pit interrupt(scheduler) automatically boot the os

    //dis_terminal = 1;
    // derefference_invalid_address();
    // derefference_valid_address();
